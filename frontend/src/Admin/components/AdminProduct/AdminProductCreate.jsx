...
const AdminProductCreate = () => {
  ...
  const handleCreateProductClick = () => {
    const productCreateRequest = {
      ...
      categoryId: parseInt(dataCategory), // Giả sử dataCategory là ID dưới dạng chuỗi
    };

    const productItems = combinedData.map((item) => ({
      price: item.price,
      quantityInStock: item.quantity,
      productImage: item.image, // Giả sử 'image' là trường tương ứng trong combinedData
      productConfigurations: [
        {
          variationName: "Size",
          variationOption: item.size,
        },
        {
          variationName: "Color",
          variationOption: item.color,
        },
      ],
    }));
    ...
    <div style={{ display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ marginTop: "20px" }}>
        ...
        >
          ...
          <Group title={"Hạng mục"} onDataChange={setDataCategory} />
        </div>

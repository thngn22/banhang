...
export const getCartItems = async (accessToken, axiosJWT) => {
  const res = await axiosJWT.get(`${process.env.REACT_APP_API_URL}cart/`, {
    headers: {
      Authorization: `Bearer ${accessToken}`,
    },
  });
...
export const getDistance = async (data, accessToken, axiosJWT) => {
  const res = await axiosJWT.post(
    `${process.env.REACT_APP_API_URL}cart/distance`,
    data,
    {
      headers: {
        Authorization: `Bearer ${accessToken}`,

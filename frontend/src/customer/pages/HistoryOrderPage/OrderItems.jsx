...
const OrderItems = () => {
  ...
  const mutationRating = useMutationHook((data) => {
    const res = OrderService.ratingProductOrdered(detailOrder?.id, data, auth.accessToken);
    return res;
  });
  const getRating = async () => {
    const res = await OrderService.getRatingProductOrdered(detailOrder?.id, auth.accessToken);
    return res;
  };
  ...
  return (
    <div>
      {detailOrder?.orderItems.map((orderItem, index) => {
        const matching = rating?.find(x => x?.productId === orderItem?.productId)
        return <div key={index}>
          <div className="p-3 shadow-lg border rounded-md mb-3">
            ...
            {detailOrder?.statusOrder === 'HOAN_TAT' && matching === undefined && <div>
              <h1 className='text-[16px] font-semibold'>Đánh giá sản phẩm</h1>
              <div className='mt-2'>
                <StarRating notSelect={true} onSelectedStar={onSelectedStar} ratingStar={0}></StarRating>
              </div>
              <div className='my-2'>
                <Input value={feedbackText} onChange={(e) => setFeedbackText(e.target.value)} placeholder="Đánh giá của bạn" />
              </div>
              <div>
                <UploadImage isEdit={false} onImageChange={handleImageChange} />
              </div>
            ...
            {detailOrder?.statusOrder === 'HOAN_TAT' && matching !== undefined && <div>
              ...
              <div className='mt-2'>
                <StarRating className="cursor-none" onSelectedStar={onSelectedStar} notSelect={false} ratingStar={matching?.rating}></StarRating>
              </div>
              <div className='my-2'>
                <Input value={matching?.feedback} />
              </div>
              <div>
                <img src={`${rating?.imageFeedback ?? ""}`} alt={`${rating?.imageFeedback ?? ""}`} />
              </div>
